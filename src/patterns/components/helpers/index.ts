...
export const toFixed = (n: number, prec: number): number => {
...
export const formatNumber = (num: number, decimals: number = 2, decPoint: string = '.', thousandsSep: string = ''): string => {
    ...
    const s: string[] = (prec ? toFixed(n, prec) : Math.round(n)).toString().split('.');
